...
async def send_meta_whatsapp_message(to: str, message: str):
    missing_vars = missing_env_vars("WHATSAPP_TOKEN", "WHATSAPP_PHONE_ID")
    if missing_vars or not WHATSAPP_API_URL:
        logger.error(
            "WhatsApp send skipped because required environment variables are missing: %s",
            ", ".join(missing_vars),
        )
        return
...
async def lifespan(app: FastAPI):
    global product_context, groq_client, startup_issues
    logger.info("Starting Edmund Lungis WhatsApp Assistant...")
    startup_issues = []

    missing_groq = missing_env_vars("GROQ_API_KEY")
    if missing_groq:
        startup_issues.append(f"Missing environment variables: {', '.join(missing_groq)}")
        logger.warning(startup_issues[-1])
        groq_client = None
    else:
        try:
            ...
            groq_client = None
            startup_issues.append(f"Groq initialization failed: {e}")
            logger.error(startup_issues[-1])

    try:
        missing_supabase = missing_env_vars("SUPABASE_URL", "SUPABASE_KEY")
        if missing_supabase:
            issue = f"Missing environment variables: {', '.join(missing_supabase)}"
...
async def health():
    return {
        "status"          : "healthy" if not startup_issues else "degraded",
    ...
    try:
        ...
        if user_text.lower() in greetings:
            ...
            if groq_client is None:
                reply = "âš ï¸ AI service is not configured right now. Please try again shortly."
            else:
